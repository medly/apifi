...
class ApiMethodBuilderTest : StringSpec({
    ...
    "generate api method based on spec operation method and url" {
        ...
        forAll(
                row(ApiMethodBuilder().methodFor("/pets", operation1, testModelMapping(), emptyList()), "@io.micronaut.http.annotation.Get(value = \"/pets\")\n" +
                        "fun getOpName() = HttpResponse.ok(controller.getOpName())\n"),
                row(ApiMethodBuilder().methodFor("/pets", operation2, testModelMapping(), emptyList()), "@io.micronaut.http.annotation.Post(value = \"/pets\")\n" +
                        "fun postOpName() = HttpResponse.ok(controller.postOpName())\n"),
                row(ApiMethodBuilder().methodFor("/pets/{petId}", operation3, testModelMapping(), emptyList()), "@io.micronaut.http.annotation.Get(value = \"/pets/{petId}\")\n" +
    ...
    "generate api method with query, path and header params" {
        ...
        val api = ApiMethodBuilder().methodFor("/pets", operation, testModelMapping(), emptyList())
    ...
    "should add security parameters if required" {
        val operation = Operation(PathItem.HttpMethod.POST, "createPet", emptyTags(), emptyParams(), Request("Pet", listOf("application/json", "text/plain")), listOf(Response("200", "PetResponse")))
        val api = ApiMethodBuilder().methodFor("/pets", operation, testModelMapping(), listOf(SecurityDefinition("openid", SecurityDefinitionType.OIDC)))

        api.parameters.map { it.toString() } shouldContainExactlyInAnyOrder
            listOf("@io.micronaut.http.annotation.Body body: models.Pet", "authentication: io.micronaut.security.authentication.Authentication?")    }
