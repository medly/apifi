...
class ApiMethodBuilderTest : StringSpec({
    ...
    "generate api method based on spec operation method and url" {
        ...
        forAll(
                row(ApiMethodBuilder().methodFor("/pets", operation1, testModelMapping()), "@io.micronaut.http.annotation.Get(value = \"/pets\")\n" +
                        "fun getOpName() = HttpResponse.ok(controller.getOpName())\n"),
                row(ApiMethodBuilder().methodFor("/pets", operation2, testModelMapping()), "@io.micronaut.http.annotation.Post(value = \"/pets\")\n" +
                        "fun postOpName() = HttpResponse.ok(controller.postOpName())\n"),
                row(ApiMethodBuilder().methodFor("/pets/{petId}", operation3, testModelMapping()), "@io.micronaut.http.annotation.Get(value = \"/pets/{petId}\")\n" +
    ...
    "generate api method with query, path and header params" {
        ...
        val api = ApiMethodBuilder().methodFor("/pets", operation, testModelMapping())
